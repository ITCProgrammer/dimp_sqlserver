...
if ( ! function_exists('generatehtml'))
{
	...
	function inputan($type,$names,$class,$placeholder,$required,$values,$tags)
        ...
        function inputan2($type,$names,$class,$placeholder,$required,$values,$tags,$reado)
        {       
            if(empty($tags))
            {
                $tagtemp="";
            ...
            $requred=$required==0?'':"required='required'";
			$readonly=$reado==0?'':"readonly";
            return "<div class='$class'><input type='$type' name='$names' placeholder='$placeholder' class='form-control' $readonly $requred value='$values' $tagtemp></div>";
        }
	...
	function buatcombo2($nama,$table,$class,$field,$pk,$kondisi,$tags,$kode)
    {
        $CI =& get_instance();
        $CI->load->model('mcrud');
        if(empty($tags))
        ...
        foreach ($data as $r)
        {
            echo" <option value=".$r->$pk.">".strtoupper($r->$kode)." ".strtoupper($r->$field)."</option>";
        }
            echo"</select></div>";
    }
	
	function buatcombo3($nama,$table,$class,$field,$pk,$kondisi,$tags)
    {
        $CI =& get_instance();
        $CI->load->model('mcrud');
        if(empty($tags))
	...
	function editcombo2($nama,$table,$class,$field,$pk,$kondisi,$tags,$value)
    {
        $CI =& get_instance();
        $CI->load->model('mcrud');
        if(empty($tags))
