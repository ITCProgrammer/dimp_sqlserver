    ...
    <table width="100%" border="1">
        ...
        <tr>
            ...
            <td style="text-align: center;">
                <?php
                    $q_stok_min = "SELECT
                                        COALESCE(floor(SAFETYSTOCK), 0) AS SAFETYSTOCK 
                                    FROM
                                        ITEMWAREHOUSELINK s
                                    WHERE
                                        TRIM(s.SUBCODE01) || '-' || TRIM(s.SUBCODE02) || '-' ||
                                        TRIM(s.SUBCODE03) || '-' || TRIM(s.SUBCODE04) || '-' ||
                                        TRIM(s.SUBCODE05) || '-' || TRIM(s.SUBCODE06) = '$row_barang[KODE_BARANG]'";
                    $result_stok_min = db2_exec($conn1, $q_stok_min);
                    $row_stok_min = db2_fetch_assoc($result_stok_min);
                    $stock_min = $row_stok_min['SAFETYSTOCK'] == 0 ? '' : $row_stok_min['SAFETYSTOCK'];
